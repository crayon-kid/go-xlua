...
/*
#cgo CFLAGS: -I ${SRCDIR}/xlua

#cgo linux LDFLAGS: -L ${SRCDIR}/../bin/ -Wl,-rpath="./" -lxlua -lm

#cgo darwin pkg-config: xlua m

#cgo freebsd LDFLAGS: -lxlua -lm

#cgo windows LDFLAGS: -L ${SRCDIR}/xlua/lib/ -lxlua
...
// lua_typename
func (L *State) Typename(tp LuaValType) string {
...
// Registry dll function
func (L *State) RegistryDllFunc() {
	C.clua_registry_dll(L.s)
}

func (L *State) PushTableKV(k, v interface{}) {
	switch val := k.(type) {
	case int:
	...
	case string:
		L.PushString(val)
	default:
		panic(fmt.Sprintf("PushTableKV|Undefine Key Type : %v", reflect.TypeOf(k)))
	}
	...
	case LUA_TableType:
		L.NewTable()
	default:
		panic(fmt.Sprintf("PushTableKV|Undefine Value Type : %v", reflect.TypeOf(v)))
	}
	L.SetTable(-3)
}
