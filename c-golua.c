...
{
	unsigned int* fidptr = (unsigned int *)lua_newuserdatauv(L, sizeof(unsigned int),1);
	*fidptr = fid;
...
{
	unsigned int* iidptr = (unsigned int *)lua_newuserdatauv(L, sizeof(unsigned int),1);
	*iidptr = iid;
...
{
	return lua_newstate(&allocwrapper,goallocf,0);
}
...
//注入ddl中的函数
void clua_reg(lua_State* L,const char *k,lua_CFunction f)
{
...
void clua_registry_dll(lua_State* L)
{
    //reg third party here 
    //example:  clua_reg(L,"fixmath",&luaopen_fixmath);
}
